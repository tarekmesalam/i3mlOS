...
| **1 — Write now (note on hashing)** | SHA-256 is ours, and deliberately not an exception to the rule below. The Tier-2 carve-out exists for *secrecy* — constant-time signature and key-exchange code, where a subtle bug is exploitable and unreviewable by one person. The journal hash is *integrity*: a fixed, published function, handling no secrets, whose correctness is decided by published test vectors rather than by an audit. |
